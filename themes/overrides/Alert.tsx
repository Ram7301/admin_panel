...
export default function Alert(theme: ThemeArg) {
  const { vars } = theme;

  const standardVariant = ({ ownerState }: { ownerState: { severity?: string } }) => {
  ...
  const outlinedVariant = ({ ownerState }: { ownerState: { severity?: string } }) => {
    if (ownerState.severity === 'primary' || ownerState.severity === 'secondary') {
      const paletteColor = vars.palette[ownerState.severity];
  ...
  const filledVariant = ({ ownerState }: { ownerState: { severity?: string } }) => {
    if (ownerState.severity === 'primary' || ownerState.severity === 'secondary') {
      const paletteColor = vars.palette[ownerState.severity];
