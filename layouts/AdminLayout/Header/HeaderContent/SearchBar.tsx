...
export default function SearchBar() {
  ...
  const openPopper = (_event?: unknown) => {
      ...
      <OutlinedInput
        ...
        endAdornment={
          <InputAdornment position="end">
            <Stack
              direction="row"
              sx={{ gap: 0.25, opacity: 0.8, alignItems: 'center', color: 'grey.600', '& svg': { color: 'inherit' } }}
            >
              <IconCommand />
                  ...
                  <List disablePadding>
                    ...
                    {listCotent.map((list, item) => (
                      <Fragment key={item}>
                        ...
                        {list.items.map((listItem, index) => renderListItem(listItem, index))}
                      </Fragment>
                    ))}
                  </List>
                )}
